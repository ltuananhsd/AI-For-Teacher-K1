...
For a complete implementation, see the tutorial below. ## [Tutorial: Build a multi-source knowledge base with routingBuild a router that queries GitHub, Notion, and Slack in parallel, then synthesizes results into a coherent answer. Covers state definition, specialized agents, parallel execution with `Send`, and result synthesis.](/oss/python/langchain/multi-agent/router-knowledge-base)
