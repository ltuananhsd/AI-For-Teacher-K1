...
function PaymentContent() {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      ...
      if (result.payment_status === 'completed' || result.enrollment_status === 'paid') {
        ...
        router.replace(`/success?id=${enrollmentId}`);
        return;
      }

      // Handle cancelled
      if (result.enrollment_status === 'cancelled') {
        if (pollIntervalRef.current) clearInterval(pollIntervalRef.current);
        if (countdownIntervalRef.current) clearInterval(countdownIntervalRef.current);
  ...
  }, [enrollmentId, fetchStatus, router]);
  ...
  const handleCancel = async () => {
    ...
    try {
      await fetch(`${API_BASE}/api/enrollments/${enrollmentId}/cancel`, {
        method: 'POST',
      });
    } catch {
      // ignore
