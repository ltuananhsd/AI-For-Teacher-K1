...
Workflow StateAgentUserWorkflow StateAgentUserStep: Get warranty statusTools: record_warranty_statusStep: Classify issueTools: record_issue_typeStep: Provide resolutionTools: provide_solution, escalate_to_human"My phone is broken""Is your device under warranty?""Yes, it's still under warranty"record_warranty_status("in_warranty")"Can you describe the issue?""The screen is cracked"record_issue_type("hardware")"Here's the warranty repair process..."
...
For a complete implementation, see the tutorial below. ## [Tutorial: Build customer support with handoffsLearn how to build a customer support agent using the handoffs pattern, where a single agent transitions between different configurations.](/oss/python/langchain/multi-agent/handoffs-customer-support)
