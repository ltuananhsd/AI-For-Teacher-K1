...
async def init_db():
    ...
    async with pool.acquire() as conn:
        ...
        # --- Phase 2 migration: source + page_id tracking ---
        await conn.execute("ALTER TABLE sessions ADD COLUMN IF NOT EXISTS source TEXT DEFAULT 'web'")
        await conn.execute("ALTER TABLE sessions ADD COLUMN IF NOT EXISTS page_id TEXT DEFAULT NULL")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_sessions_source ON sessions(source)")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_sessions_source_updated ON sessions(source, updated_at DESC)")
        # Backfill: existing fb_ sessions get source='facebook'
        await conn.execute("""
            UPDATE sessions SET source = 'facebook' WHERE id LIKE 'fb_%' AND source = 'web'
        """)

        # --- Phase 3 migration: multi-bubble toggle per Facebook page ---
        await conn.execute(
            "ALTER TABLE facebook_pages ADD COLUMN IF NOT EXISTS multi_bubble_enabled INTEGER DEFAULT 1"
        )
...
async def create_session(session_id=None, title="New Chat", source='web', page_id=None):
    """Create session if not exists (idempotent). source='web'|'facebook'."""
    if not session_id:
        session_id = str(uuid.uuid4())
    async with get_pool().acquire() as conn:
        await conn.execute(
            """INSERT INTO sessions (id, title, source, page_id)
               VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING""",
            session_id, title, source, page_id,
        )
    return session_id
...
async def get_sessions_by_source(source: str, cursor: str = None, limit: int = 20):
    """Cursor-based paginated sessions filtered by source.
    cursor: ISO timestamp string of last session's updated_at.
    """
    pool = get_pool()
    async with pool.acquire() as conn:
        if cursor:
            rows = await conn.fetch(
                """SELECT s.*, fp.page_name
                   FROM sessions s
                   LEFT JOIN facebook_pages fp ON s.page_id = fp.page_id
                   WHERE s.source = $1 AND s.updated_at < $2::timestamp
...
async def get_facebook_pages():
    ...
    async with get_pool().acquire() as conn:
        rows = await conn.fetch(
            "SELECT id, page_id, page_name, is_active, multi_bubble_enabled, created_at FROM facebook_pages ORDER BY created_at DESC"
        )
        return [dict(r) for r in rows]
...
async def toggle_multi_bubble(row_id: str):
    """Toggle multi_bubble_enabled flag for a Facebook page."""
    async with get_pool().acquire() as conn:
        await conn.execute(
            "UPDATE facebook_pages SET multi_bubble_enabled = CASE WHEN multi_bubble_enabled = 1 THEN 0 ELSE 1 END WHERE id = $1",
            row_id,
        )


async def get_facebook_page_multi_bubble(page_id: str) -> bool:
    """Return whether multi-bubble is enabled for a given Facebook page_id (webhook use)."""
    async with get_pool().acquire() as conn:
        val = await conn.fetchval(
            "SELECT multi_bubble_enabled FROM facebook_pages WHERE page_id = $1 AND is_active = 1",
            page_id,
        )
