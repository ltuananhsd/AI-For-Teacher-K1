...
# Separator token AI uses to split reply into multiple chat bubbles
FB_BUBBLE_SEP = "|||"
...
async def send_fb_message(recipient_id: str, text: str, access_token: str):
    """Send a single text message to a Facebook user via Graph API."""
    payload = {
...
async def _mark_seen(recipient_id: str, access_token: str):
    """Send mark_seen action so user knows bot received the message."""
    try:
...
async def _send_typing(recipient_id: str, on: bool, access_token: str):
...
async def send_bubbles(recipient_id: str, text: str, access_token: str):
    """Split text on FB_BUBBLE_SEP (|||) and send each part as a separate message.
    Each bubble: mark_seen → typing_on → send.
    Falls back to single send if no separator found.
    """
    import asyncio
    parts = [p.strip() for p in text.split(FB_BUBBLE_SEP) if p.strip()]
    if not parts:
        parts = [text.strip()]

    for i, part in enumerate(parts):
        await _mark_seen(recipient_id, access_token)
        await _send_typing(recipient_id, True, access_token)
        await asyncio.sleep(0.4)  # tiny pause so typing indicator renders
        await send_fb_message(recipient_id, part, access_token)
        if i < len(parts) - 1:
            await asyncio.sleep(0.3)  # brief gap between bubbles
...
async def handle_message(
    ...
    try:
        ...
        # Acknowledge receipt immediately (mark_seen)
        await _mark_seen(sender_id, access_token)
        await _send_typing(sender_id, True, access_token)

        # Ensure session exists
        await database.create_session(session_id, title=f"FB: {message_text[:30]}", source='facebook', page_id=page_id)
        ...
        # Facebook-specific: multi-bubble instruction appended LAST
        fb_bubble_prompt = PROMPTS.get("fb_multi_bubble", "")
        if fb_bubble_prompt:
            final_prompt += f"\n\n{fb_bubble_prompt}"
        ...
        # Turn off typing, then send — multi-bubble is per-page configurable
        await _send_typing(sender_id, False, access_token)
        multi_bubble = await database.get_facebook_page_multi_bubble(page_id)
        if multi_bubble:
            await send_bubbles(sender_id, response_text, access_token)
        else:
            # Single message: strip ||| separators before sending
            single_text = response_text.replace(FB_BUBBLE_SEP, " ").strip()
            await send_fb_message(sender_id, single_text, access_token)
