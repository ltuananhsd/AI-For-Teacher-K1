    ...
    return (
        <div>
            ...
            {taskStatus && !uploading && (
                <div className={`mt-3 flex items-center gap-2 text-sm px-3 py-2 rounded-xl ${taskStatus.status === 'done' ? 'bg-green-50 text-green-700' :
            ...
            {/* Dedup stats */}
            {taskStatus?.status === 'done' && taskStatus?.dedup_stats && (
                <div className="mt-2 px-3 py-2 bg-blue-50 border border-blue-100 rounded-xl text-xs text-blue-700 space-y-0.5">
                    <p className="font-medium">Kết quả kiểm tra trùng lặp:</p>
                    <p>✅ {taskStatus.dedup_stats.unique_chunks_added} đoạn mới được thêm vào index</p>
                    {taskStatus.dedup_stats.duplicates_removed > 0 && (
                        <p>⚠️ {taskStatus.dedup_stats.duplicates_removed} đoạn trùng bị loại (độ tương đồng &gt; 92%)</p>
                    )}
                </div>
            )}
        </div>
