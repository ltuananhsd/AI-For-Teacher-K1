...
def load_prompts():
    ...
    try:
        ...
        # Optional: Facebook-specific multi-bubble prompt addendum
        fb_multi_bubble = ""
        fb_bubble_path = f"{PROMPTS_DIR}/facebook-multi-bubble.txt"
        if os.path.exists(fb_bubble_path):
            with open(fb_bubble_path, "r", encoding="utf-8") as f:
                fb_multi_bubble = f.read().strip()
        ...
        PROMPTS = {
            ...
            "fb_multi_bubble": fb_multi_bubble,
        }
...
# ---------------------------------------------------------------------------
# Response cache — in-memory TTL dict for /api/v1/chat
# key = md5(message.lower()), value = {"response": str, "ts": float}
# ---------------------------------------------------------------------------
_response_cache: dict = {}
_CACHE_TTL = 300    # 5 minutes
_CACHE_MAX = 1000   # max entries before LRU eviction
...
def _cache_get(key: str):
    ...
    if entry:
        del _response_cache[key]   # expired — remove
    return None
...
def _cache_set(key: str, response: str):
    ...
    _response_cache[key] = {"response": response, "ts": _time.time()}
...
@app.get("/api/v1/history/facebook")
async def get_fb_history(cursor: str = None, limit: int = 20):
    """Get Facebook-source sessions, cursor-paginated."""
    return await database.get_sessions_by_source('facebook', cursor, limit)

@app.get("/api/v1/history/test")
async def get_test_history(cursor: str = None, limit: int = 20):
    """Get web-source (test bot) sessions, cursor-paginated."""
    return await database.get_sessions_by_source('web', cursor, limit)
...
# --- In-memory page info cache (1hr TTL) ---
_page_info_cache: dict = {}  # {page_id: {"data": {...}, "ts": float}}

@app.get("/api/v1/facebook/pages/{page_db_id}/info")
async def fb_page_info(page_db_id: str):
    """Fetch Facebook Page public info via Graph API. Cached 1hr in memory."""
    pool = database.get_pool()
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT page_id, page_access_token FROM facebook_pages WHERE id = $1", page_db_id
        )
    if not row:
    ...
    if cached and time.time() - cached["ts"] < 3600:
        return cached["data"]
    try:
        token = facebook_handler.decrypt_token(row["page_access_token"])
        async with httpx.AsyncClient(timeout=15) as client:
            resp = await client.get(
                f"https://graph.facebook.com/v19.0/{page_id}",
                params={"fields": "name,fan_count,category,about", "access_token": token}
            )
        if resp.status_code != 200:
            raise HTTPException(status_code=502, detail=f"Graph API error: {resp.text}")
        data = resp.json()
        _page_info_cache[page_id] = {"data": data, "ts": time.time()}
        return data
...
@app.get("/api/v1/facebook/pages/{page_db_id}/conversations/count")
async def fb_page_conversation_count(page_db_id: str):
    """Return number of FB chat sessions for this page."""
    pool = database.get_pool()
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT page_id FROM facebook_pages WHERE id = $1", page_db_id
        )
        if not row:
            raise HTTPException(status_code=404, detail="Page not found")
        count = await conn.fetchval(
            "SELECT COUNT(*) FROM sessions WHERE page_id = $1 AND source = 'facebook'",
            row["page_id"],
        )
    return {"count": count}
...
@app.put("/api/v1/facebook/pages/{page_db_id}/toggle-multi-bubble")
async def fb_toggle_multi_bubble(page_db_id: str):
    """Toggle multi-bubble (|||) splitting for a specific Facebook page."""
    await database.toggle_multi_bubble(page_db_id)
    return {"ok": True}

@app.delete("/api/v1/cache")
async def clear_response_cache():
    """Admin: clear the in-memory response cache."""
...
async def chat_endpoint(request: ChatRequest):
    ...
    # Cache check — only for non-first messages (skip new sessions with welcome msg)
    if msg_count > 0:
        _ck = _cache_key(request.message)
        _cached = _cache_get(_ck)
        if _cached:
            logger.info(f"[{request.session_id}] Cache HIT")
            await database.add_message(request.session_id, "user", request.message)
            await database.add_message(request.session_id, "bot", _cached)
        ...
        async def generate():
            ...
            try:
                ...
                if full_response:
                    ...
                    # Store in response cache for subsequent identical questions
                    if _ck:
